...
[![CI](https://github.com/Auenchanters/Virtual-PromptWars-week-2/actions/workflows/ci.yml/badge.svg)](https://github.com/Auenchanters/Virtual-PromptWars-week-2/actions/workflows/ci.yml)
...
| **Security** | [app/security.py](app/security.py): CSP (no `unsafe-inline` / `unsafe-eval` / wildcards), HSTS (only on https), COOP, CORP, X-Frame-Options, Permissions-Policy, Referrer-Policy, X-Request-ID, body-size 413, **`Server` header stripped**, **`Cache-Control: no-store`** on every sensitive `/api/*` POST. **RFC 9116** disclosure file at [/.well-known/security.txt](app/routers/info.py) and a [/robots.txt](app/routers/info.py) keeping crawlers out of the API surface. Per-IP sliding-window limiter with `Retry-After`. Pydantic v2 validation on every input. Secret Manager for `GEMINI_API_KEY` and `GOOGLE_MAPS_API_KEY`. **Cloud DLP** redacts PII (phone / email / Aadhaar / PAN / credit card) before any analytics row leaves the process. **`pip-audit`** runs in CI for dependency CVEs. | [tests/test_security_headers.py](tests/test_security_headers.py) enumerates every header, body-size, invalid-content-length, HSTS toggle, server-header strip, security.txt + robots.txt, no-store on `/api/chat`, and asserts CSP semantics. [tests/test_dlp.py](tests/test_dlp.py) exercises the redactor. |
...
| **Testing** | **170 tests across 16 files**, fully offline (no Google SDK calls — every external dep is a `Protocol` swapped via FastAPI dependency overrides). Coverage **89.46 %** with a **`--cov-fail-under=88`** gate enforced in CI. Direct unit tests for every helper in [app/deps.py](app/deps.py) ([tests/test_deps.py](tests/test_deps.py)) and every Pydantic validator in [app/models.py](app/models.py) ([tests/test_models.py](tests/test_models.py)). The lone `# pragma: no cover` on the streaming error path was removed. Per-language tests parametrize across **all 13 Indian languages**. **Test-order isolation guard**: an `autouse` fixture in [tests/conftest.py](tests/conftest.py) resets every singleton (translator, speaker, analytics, redactor, places, secrets) and clears `app.dependency_overrides` between tests. **LRU eviction boundaries** explicitly tested for all three cache types (`_LruCache`, `_LruStringCache`, `_LruBytesCache`). | `pytest --cov=app --cov-report=term --cov-fail-under=88`. CI in [.github/workflows/ci.yml](.github/workflows/ci.yml). |
...
tests/
  ...
  conftest.py
.github/workflows/ci.yml    # ruff + mypy + pytest --cov-fail-under=88 + pip-audit on PR
